...
tree_mos <- read.iqtree("03_MosAIC_Phylogeny/270423_concatenated_16s_fileappend_final_with_outgroup_aln_trimal.fa.contree")
tree_mos_root <- root(tree_mos, outgroup = "16S_rRNA")

### Load the meta data of names and sample IDs corresponding to the nodes of the tree 
dat1_mos <- read.csv("03_MosAIC_Phylogeny/MMO142ADJ_240223_tree_tibble_join_td_select.csv")
dat1_mos <- dat1_mos[1:142,]

### Load the metadata of the full table 
dat2_mos <- read.csv("03_MosAIC_Phylogeny/MMO142ADJ_240223_071222_tree_tibble_drep_join_full_table.csv")
dat2_mos <- dat2_mos[1:142,]


### Make the first tree - fan layout and include bootstrap values
p <- ggtree(tree_mos, layout="fan", size=0.2, open.angle=20) + 
...
pv2 <- ggtree(tree_mos_root, layout="circular", size=0.2, open.angle=20) + 
  geom_point2(aes(subset=!isTip & UFboot > 80, x = branch, col = UFboot), size = 0.7, na.rm = T) + 
  scale_color_gradientn(colors = met.brewer("Pillement", n = 3))
...
ggsave(pv2_2, filename = "plots/PlotsFinal/Figure2_ALT_LegendKey.pdf", width = 14, heigh = 35)
  ...
  #guide=guide_legend(keywidth = 0.3, 
  #keyheight = 0.3, order=4))
  geom_treescale(fontsize=2, linesize=0.1, x=0.5, y=-2) +
...
ggsave(filename = "plots/PlotsFinal/Figure_2_MosAIC_dRep_Cluster_Phylogeny_Circular_Bootstraps.pdf", plot = pv2_3, height = 10, width = 22)
